module Persistable
  module ClassMethods
    ...
    def public_attributes
      self.attributes.keys.reject{|k| k == :id}
    end
    ...
    def create_table
      create_statement = self.attributes.collect{ |k, v| "#{k} #{v}"}.join(", ")
      sql = <<-SQL
    ...
    def find_by_id(id)
      ...
      SQL
      row = self.db.execute(sql, id).flatten
      self.new_from_db(row)
    end
  end
  ...
  module InstanceMethods
    ...
    def insert
      ...
      sql = <<-SQL
        INSERT INTO #{self.class.table_name} (#{sql_for_insert}) VALUES (#{question_marks});
      SQL
